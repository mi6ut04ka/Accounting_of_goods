...
@section('content')
    <div class="container mt-5">
        ...
        @foreach ($raws as $categoryName => $categoryRaws)
            <h2 class="mt-4">{{ $categoryName }}</h2>

            @if ($categoryRaws->count())
                <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
                    @foreach ($categoryRaws as $raw)
                        <div class="col">
                            <div class="card shadow-sm h-100">
                                <div class="card-img-top text-center bg-light p-3">
                                    @if ($raw->photo)
                                        <img src="{{ asset('https://s3.regru.cloud/aromosa/' . $raw->photo->url) }}" class="img-fluid rounded" alt="Фото продукта" style="max-height: 200px; object-fit: contain;">
                                    @else
            ...
            @else
                <p class="text-muted text-center">В этой категории нет сырья.</p>
            @endif
        @endforeach
    </div>
@endsection
